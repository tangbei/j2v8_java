...
public class J2V8Util {
     ...
     * @throws Exception
     */
    public static String supplyAsync(InputStream inputStream) throws Exception {
        CompletableFuture<String> future = CompletableFuture.supplyAsync(() -> {
            String jsonJs = getFileContent(inputStream);
            return jsonJs;
        });
        return future.get();
    }
     ...
     * @return 返回字符串
     */
    public static V8Object v8ObjectFunction(V8 v8, String name, V8Array array) {
        V8Object result = null;
        try {
            if (v8 != null && !v8.isReleased()) {
                if (null != array) {
                    result = v8.executeObjectFunction(name, array);
                    array.release();
                } else {
                    result = v8.executeObjectFunction(name, null);
                }
            }
        } catch (Exception e) {
            e.printStackTrace();
